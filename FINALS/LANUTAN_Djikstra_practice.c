...
set* Dijkstras(graphType graph, int start) {
	set* small = (set*)malloc(sizeof(int) * MAX);
	set visited = {0};
	int x, y, u, v, head, tail, minNdx;
	...
	if (small != NULL){
		...
		for (x = 0 ; x < MAX ; x++){
			minNdx = -1;
			for (y = 0 ; y < MAX ; y++){
				if (!visited[y] && (minNdx == -1 || (*small)[y] < (*small)[minNdx])){
					minNdx = y;
				}
			}
			visited[minNdx] = 1;
			...
			for (y = 0 ; y < MAX ; y++){
				if (!visited[y] && graph[minNdx][y] != INF){
					if ((*small)[minNdx] + graph[minNdx][y] < (*small)[y]){
						(*small)[y] = (*small)[minNdx] + graph[minNdx][y];
					}
				}
			}
		}
