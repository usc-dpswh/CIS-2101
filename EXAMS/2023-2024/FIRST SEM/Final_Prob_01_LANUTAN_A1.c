...
{ 
   int count;
	...
	ProdList head 	= NULL;
	ProdList* trav 	= NULL;
	ProdList newNode = NULL;
	for (x = 0 ; x < count ; x++){
		for (trav = &head ; *trav != NULL ; trav = &(*trav)->plink){};
		newNode = (ProdList)malloc(sizeof(struct pnode));
		if (newNode != NULL){
			newNode->prod = data[x];
			newNode->plink = NULL;
			*trav = newNode;
//			printf("\nInserted [%s] with ID [%s]", newNode->prod.prodDesc.name, newNode->prod.prodID);
		}
	}
	return head;
}
...
void makeListToSet(ProdList L)
{
    ...
    ProdList check;         // Basis
    ProdList* trav;         // To look for duplicates based on basis
    ProdList temp;          // To delete duplicate
    
    for (check = L ; check != NULL ; check = check->plink) {
        for (trav = &check->plink; *trav != NULL; ) {
            if (strcmp(check->prod.prodID, (*trav)->prod.prodID) == 0) {
                temp = *trav;
                *trav = temp->plink;
                printf("\nDeleted duplicate of %s", check->prod.prodID);
                free(temp);
            } else {
                trav = &(*trav)->plink;
            }
